...
import { isInstanceScript, isModuleScript, isScript, isScriptTS } from "./script.js";
...
<script lang="ts">
	...
	let y: number;
		...
		`;
		const node = parse_and_extract<AST.Script>(code, "Script");
		expect(isScriptTS(node)).toBe(true);
	});
...
<script lang="ts">
	...
	it("returns false for script without `lang` attribute", ({ expect }) => {
		const code = "<script>let x = 1;</script>";
		const node = parse_and_extract<AST.Script>(code, "Script");
		expect(isScriptTS(node)).toBe(false);
	});
});
